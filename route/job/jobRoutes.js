...
fastify.get(
  ...
  async (req, reply) => {
    ...
    // ───────── helpers ─────────
    const normalizeArray = (v) => (Array.isArray(v) ? v.filter(Boolean) : v ? [v] : []);
    const toObjectId = (v) => { try { return v ? new mongoose.Types.ObjectId(String(v)) : null; } catch { return null; } };
    ...
    try {
      ...
      const userSkills = (user?.skills || []).map((s) => (s?._id || s)?.toString()).filter(Boolean);
      const userLocation = user?.location
        ? {
            provinsi: user.location.provinsi?._id?.toString() || user.location.provinsi?.toString() || "",
            kabupaten: user.location.kabupaten?._id?.toString() || user.location.kabupaten?.toString() || "",
            kecamatan: user.location.kecamatan?._id?.toString() || user.location.kecamatan?.toString() || "",
          }
        : null;
      ...
      const locationQCleaned =
        locationQ.provinsi || locationQ.kabupaten || locationQ.kecamatan ? locationQ : null;
      ...
      if (interactedJobIds.length) scopeMatch._id = { $nin: interactedJobIds.map(toObjectId).filter(Boolean) };
      ...
      if (filterActive) {
        ...
        if (minSalaryActive)    scopeMatch["salaryRange.min"] = { $gte: minSalary };
        if (locationQCleaned) {
      ...
      const userMatchOr = [];
      if (user) {
        if (userSkills.length) userMatchOr.push({ requiredSkills: { $in: userSkills.map(toObjectId).filter(Boolean) } });
        if (userLocation?.provinsi)  userMatchOr.push({ "location.provinsi": toObjectId(userLocation.provinsi) });
        if (userLocation?.kabupaten) userMatchOr.push({ "location.kabupaten": toObjectId(userLocation.kabupaten) });
        if (userLocation?.kecamatan) userMatchOr.push({ "location.kecamatan": toObjectId(userLocation.kecamatan) });
        userMatchOr.push({ workType: "remote" });
      }

      const postLookupKeyword = keywordActive
        ? { $or: [ { title: { $regex: keyword, $options: "i" } }, { "company.companyName": { $regex: keyword, $options: "i" } } ] }
        : null;

      // ───────── pretty header ─────────
      console.log(
        `\n┌──────────────────────── /job-feed ────────────────────────┐\n` +
        `│ time: ${nowIso()}   page: ${page}  limit: ${limit}  actor: ${user ? `user:${userId}` : "guest"}\n` +
        `│ mode: ${filterActive ? "FILTERED (strict scope)" : "UNFILTERED (global)"}\n` +
        `├──────────────────────────────── scope (query-only) ───────┤\n` +
        `│ skills    : ${JSON.stringify(skillsQ)}\n` +
        `│ location  : ${locationQCleaned ? JSON.stringify(locationQCleaned) : "[]"}\n` +
        ...
        `│ userLoc   : ${userLocation ? JSON.stringify(userLocation) : "(none)"}\n` +
        `└───────────────────────────────────────────────────────────┘`
      );
      ...
      const countWithKeyword = async (match) => {
        const pipe = [
          { $match: match },
          { $lookup: { from: COMPANY_COLLECTION, localField: "company", foreignField: "_id", as: "company" } },
      ...
      const buildPipeline = (match, { skip = 0, take = 10, randomize = false } = {}) => {
        const pipe = [
          { $match: match },
          ...
          { $unwind: { path: "$company", preserveNullAndEmptyArrays: true } },
          // required skills
          { $lookup: { from: "skills", localField: "requiredSkills", foreignField: "_id", as: "requiredSkills" } },
          // LOCATION POPULATE
          { $lookup: { from: PROVINSI_COLLECTION,  localField: "location.provinsi",  foreignField: "_id", as: "provinsiDoc" } },
          { $unwind: { path: "$provinsiDoc",  preserveNullAndEmptyArrays: true } },
          { $lookup: { from: KABUPATEN_COLLECTION, localField: "location.kabupaten", foreignField: "_id", as: "kabupatenDoc" } },
          { $unwind: { path: "$kabupatenDoc", preserveNullAndEmptyArrays: true } },
          { $lookup: { from: KECAMATAN_COLLECTION, localField: "location.kecamatan", foreignField: "_id", as: "kecamatanDoc" } },
          { $unwind: { path: "$kecamatanDoc", preserveNullAndEmptyArrays: true } },
          ...
          {
            $addFields: {
              companyRatingAverage: { $ifNull: ["$company.rating.average", { $avg: "$companyReviews.rating" }] },
              companyRatingCount:   { $ifNull: ["$company.rating.count",   { $size: "$companyReviews" }] },
              location: {
                provinsi:  { _id: { $ifNull: ["$provinsiDoc._id",  "$location.provinsi"]  }, name: "$provinsiDoc.name"  },
                kabupaten: { _id: { $ifNull: ["$kabupatenDoc._id", "$location.kabupaten"] }, name: "$kabupatenDoc.name" },
                kecamatan: { _id: { $ifNull: ["$kecamatanDoc._id", "$location.kecamatan"] }, name: "$kecamatanDoc.name" },
              }
            },
          },
          // clean temp fields
      ...
      if (matchedTake > 0) {
        matchedPage = await JobPost.aggregate(buildPipeline(matchedMatch, {
          skip: matchedSkip,
          take: matchedTake,
          randomize: false,
        }));
      ...
      const shouldRandomizeNon = filterActive && matchedTake === 0 && randomizeNonWhenNoMatches;
      if (nonTake > 0) {
        nonPage = await JobPost.aggregate(buildPipeline(nonMatch, {
      ...
      if (!filterActive && combinedTotal === 0 && (matchedPage.length + nonPage.length) === 0) {
        console.log("⚠️  no items globally → fallback to latest active");
        const latest = await JobPost.aggregate(buildPipeline(
          { isActive: true, ...(interactedJobIds.length ? { _id: { $nin: interactedJobIds.map(toObjectId).filter(Boolean) } } : {}) },
        ...
        pageJobs = pageJobs
          .map((job) => {
            ...
            const { score, reasons } = calculateMatchScore(job, user, {
              location: {
                provinsi: (userLocation?.provinsi || userLoc.provinsi?._id)?.toString(),
                kabupaten: (userLocation?.kabupaten || userLoc.kabupaten?._id)?.toString(),
                kecamatan: (userLocation?.kecamatan || userLoc.kecamatan?._id)?.toString(),
              },
              workType: workTypeQ,
              minSalary: minSalaryActive ? minSalary : undefined,
              industries: industriesQ,
            });
            const isSaved = user.savedJobs?.some((e) => String(e.job) === String(job._id));
            return {
              ...job,
              matchScore: score,
              whyThisJob: reasons,
      ...
      const sampleMatched = matchedPage.slice(0, 3).map((j) => `${j.title} · ${j.company?.companyName || "-"} · ${j.location?.kabupaten?.name || "-"}`);
      const sampleNon     = nonPage.slice(0, 3).map((j) => `${j.title} · ${j.company?.companyName || "-"} · ${j.location?.kabupaten?.name || "-"}`);

      console.log(
        `┌──────────────────────────── result ────────────────────────────┐\n` +
        `│ page=${page}  returned=${pageJobs.length}/${limit}  took=${tookMs}ms\n` +
        `│ matched: total=${matchedTotal} · thisPage=${matchedPage.length}  ex: ${sampleMatched.join(" | ") || "-" }\n` +
        `│ non    : total=${nonTotal}    · thisPage=${nonPage.length}      ex: ${sampleNon.join(" | ") || "-" }\n` +
        `└───────────────────────────────────────────────────────────────┘\n`
      );

      // =============== 12) RESPOND ===============
      const totalForClient = filterActive ? (matchedTotal + nonTotal) : (matchedTotal + nonTotal || (matchedPage.length + nonPage.length));
